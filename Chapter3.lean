...
-- it is important that these Prop variables are implicit arguments
-- otherwise applying some lemma inline may need explicit arguments.
-- see below.
...
/- associativity of And (∧) the programmers way. Initially I
tried to factor out the left and right lemma but got weird errors due to the
propositional variables not being declared as implicit! One to watch for. -/
